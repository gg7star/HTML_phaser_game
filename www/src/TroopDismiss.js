...
var TroopDismissState = {
  ...
  preload: function() {
    ...
    common.getCommonUI('troopDismissUI', this);
    ...
    jsonFile.onFileComplete.add(function(progress, cacheKey, success, totalLoaded, totalFiles) {
      ...
      if(cacheKey == 'troopDismissUI') {
        troopTrainUI = this.game.cache.getJSON(cacheKey);
        console.log("=== troopDismissUI: ", troopTrainUI);
  ...
  addDismissHandler: function() {

    // Add handler of Build button.
    common.addHandlerWithText('dismissButton', this.clickedDismiss, this);
  ...
  createTroopsInput: function() {
    ...
    max_troop = troopTrainValues.troop.amount;
    ...
    console.log("======= selectedTroop.value = ", selectedTroop.value);
    this["TroopDismissStatetroopInputboxEdit"].value = "" + max_troop;

    troopDissmissState["TroopDismissStatetroopLabelText"].cameraOffset.x = troopDissmissState.game.width / 2  - troopDissmissState["TroopDismissStatetroopLabelText"].texture.width / 2;
    // Add handler for building details.
  ...
  clickedReturn: function() {
    ...
    this.initialize();
    this.game.state.start('BarracksState');

  },

  clickedDismiss: function (sprite) {

    this.GotoBarracksState();

  },

  GotoBarracksState: function() {
    
    if (troopDissmissState["TroopDismissStatetroopInputboxEdit"].value === null && troopDissmissState["TroopDismissStatetroopInputboxEdit"].value === '') {
      alert('Please input troop amount');
      return;
    }

    if (parseInt(troopDissmissState["TroopDismissStatetroopInputboxEdit"].value) > Math.floor(max_troop)) {
    ...
    $.ajax({
      type: 'POST',
      url: troopDissmissState.game.config.settings.server + 'delete_user_private_troop',
      dataType: "JSON",
      data: { 'session':
        {
          'user_name': troopDissmissState.game.global.username,
          'troop': selectedTroop,
          'amount': troopDissmissState["TroopDismissStatetroopInputboxEdit"].value
        }
      },

      success: function(result) {
        troopDissmissState.initialize();
        troopDissmissState.game.state.start('BarracksState');
      },
  ...
  setChatDescriptionText: function(chatText) {
    ...
    common.setChatDescriptionText(chatText, troopDissmissState);
    
    common.addHandler('chatDescriptionBackground', troopDissmissState.clickedChatDescription, troopDissmissState);
    common.addHandler('crown', troopDissmissState.clickedCrown, troopDissmissState);

  },

  initialize: function() {

    troopDissmissState.chatTimerEnable = false;
    common.initializeState(troopDissmissState);
