...
var CampState = {
  ...
  preload: function() {
    ...
    common.getCommonUI('campUI', this);
    ...
    jsonFile.onFileComplete.add(function(progress, cacheKey, success, totalLoaded, totalFiles) {
      ...
      if(cacheKey == 'camp_info') {
        campValues = this.game.cache.getJSON(cacheKey);
        wasLoadedValue = true;
        if (wasLoadedUI === true) {
          console.log("=== campValues: ", campValues);
      ...
      if(cacheKey == 'campUI') {
        campUI = this.game.cache.getJSON(cacheKey);
        wasLoadedUI = true;
      }
  ...
  preloadUIFromJson: function () {
    ...
    var topLayout = common.getLayoutByID("topLayout", campUI);
    ...
    var middleLayout = common.getLayoutByID('middleLayout', campUI);
    ...
    // Set building image and name into middle layout
    var totalTroopsLayout = common.getLayoutByID('totalTroopsLayout', campUI);
    var totalAvailableTroopsImage = totalTroopsLayout.items.find(function(item){return item.id == 'totalAvailableTroopsImage';});
    totalAvailableTroopsImage.text = totalAvailableTroopsImage.prefix + ' ' + campValues.total_amount;
    ...
    var valueLayout = common.getLayoutByID("valueLayout", campUI);
    for (var i = 0; i < campValues['troops'].length; i ++) {
      var value = campValues['troops'][i];
  ...
  createTopMenus: function() {
    ...
    // Create top layout.
    totalLayoutHeight += common.createLayoutByID('topLayout', campUI, this);
  ...
  createBuildingDetails: function() {
    ...
    // Create top layout.
    totalLayoutHeight += common.createLayoutByID('middleLayout', campUI, this);
  ...
  createTotalTroopsLayout: function() {
    ...
    totalLayoutHeight += common.createLayoutByID('totalTroopsLayout', campUI, this);
  ...
  createValues: function() {
    ...
    for (var i = 0; i < valueLayout.items.length; i ++) {
      ...
      var values = campValues[itemUI.title.id];
      console.log("=== itemUI.title.id: ", itemUI.title.id, itemUI);
      console.log("=== campValues[itemUI.title.id]: ", values);
      ...
      for (var j = 0; j < values.length; j ++) {
        ...
        var troopImage = common.createImageWithText(troopPositionX, troopPositionY,
                                   ...
                                   value.id, '', null, null, 
                                   null, null, null, this);
        // x, y, imageName, width, height, name, title, color, callback, offsetX, offsetY, fontStyle, state
        // Add handler
        if (value.possible) {
  ...
  clickedTroop: function(sprite) {
    this.initialize();
    this.game.state.start('TroopTrainState', true, false, {troop: sprite.troop});
  },
